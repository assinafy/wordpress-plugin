...
3. **Artifact endpoints require account authentication.** The plugin stores artifact names
   ...
   with the API key. Browser download links point to the permission-checked WordPress proxy.
...
`::forAuth()` or `::forBearer()`** — each falls back to the absent Guzzle transport. There is
exactly one construction site, in [`ClientFactory`](src/ClientFactory.php):
...
Going through `wp_remote_request()` also buys the site's configured proxy (`WP_PROXY_*`),
`WP_HTTP_BLOCK_EXTERNAL` compliance, the site's own SSL settings, and the existing
`http_request_*` filter surface.
...
The API key is encrypted at rest with libsodium. The stored blob is
...
Nonempty string `ASSINAFY_API_KEY` and `ASSINAFY_ACCOUNT_ID` constants override their saved
...
With `ASSINAFY_API_KEY` and `ASSINAFY_ACCOUNT_ID` configured, credentials need not be stored
in the database. Defining constants does not delete previously saved credentials. Sending
and webhook account checks both honor these overrides.
...
**A webhook delivery** does exactly the same thing — see [§7](#7-webhooks). The delivery body
...
full payload from [Step 6](#step-6--track-progress), `assignment` and `pages` included.
...
[Step 3](#step-3--upload-the-pdf). Called by `SendAttempt` after it reserves a local record.
...
response in [Step 2](#step-2--resolve-each-signer-look-up-then-create).
...
[Step 2](#step-2--resolve-each-signer-look-up-then-create).
...
The send itself. Request and response in [Step 4](#step-4--create-the-assignment). Called by
...
[Step 6](#step-6--track-progress). A deleted or unknown document answers
...
[Step 6](#step-6--track-progress). No pagination; `payload` is `[]` or an object.
...
[Step 7](#step-7--deliver-the-signed-pdf).
...
[Step 1](#step-1--price-the-send-and-read-the-verdict-from-the-body).
