...
| WordPress | **6.8** | A release que estendeu o carregamento just-in-time de traduções a todos os plugins. O plugin ainda chama `load_plugin_textdomain()` no boot, porque o carregamento just-in-time nunca olha dentro do diretório do próprio plugin e o catálogo `languages/` embutido seria ignorado. |
| Testado até | 7.1 | |
...
**Uma entrega de webhook** faz exatamente a mesma coisa — veja a [§7](#7-webhooks). O corpo da
...
retornou** — o payload completo do [Passo 6](#passo-6--acompanhar-o-progresso), com `assignment` e
`pages` incluídos.
...
O upload multipart. Requisição e resposta no [Passo 3](#passo-3--enviar-o-pdf). Chamado pelo
...
[Passo 2](#passo-2--resolver-cada-signatário-buscar-e-então-criar).
...
O envio em si. Requisição e resposta no [Passo 4](#passo-4--criar-o-assignment). Chamado pelo
...
resposta no [Passo 6](#passo-6--acompanhar-o-progresso). Um documento excluído ou desconhecido
...
[Passo 6](#passo-6--acompanhar-o-progresso). Sem paginação; `payload` é `[]` ou um objeto.
...
[Passo 7](#passo-7--entregar-o-pdf-assinado).
...
[Passo 1](#passo-1--precificar-o-envio-e-ler-o-veredito-no-corpo-da-resposta).
